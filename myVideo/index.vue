<!--
 * @Author: 张嘉彬
 * @Date: 2021-10-26 09:46:48
 * @Description: https://github.com/yuelinghunyu/blog-demo/tree/master/video-player
-->
<template>
  <div class="custom-video_container" :style="{
      width: infoData.width,
      height: infoData.height
    }" :class="{'fulls-creen':videoState.screenState}" ref="custom-video_container"
       @mouseover="handleControls($event, 'start')" @mouseleave="handleControls($event, 'end')">
    <video :id='infoData.id || "custom-video_video"' class="custom-video_video" ref="custom-video"
           :loop="infoData.loopPlay"  :muted="infoData.mutePlay"
           :poster="infoData.poster">
      <source :src="infoData.videoUrl" type="video/mp4">
      <p>设备不支持</p>
    </video>
    <!-- <van-icon v-if="videoState.play" class="custom-video_play custom-video_play-pause"
              @click="pause('btn')" name="pause-circle-o" />
    <van-icon v-else class="custom-video_play custom-video_play-play" @click="play('btn')"
              name="play-circle-o" /> -->
    <!-- 控制区域背景 -->
    <div v-show='infoData.playBtn'>
      <transition name="fade">
        <div class="custom-video_control" v-show="!videoState.hideControl || !videoState.play">
          ...
          <van-icon v-else class="custom-video_play custom-video_play-play" @click="play('btn')"
                    name="play-circle-o" />
          <!-- 进度条 -->
          <div class="custom-video_control-bg" @mousedown="handlePrograssDown"
               @mousemove="handlePrograssMove" @mouseup="handlePrograssUp">
            <div class="custom-video_control-bg-outside" ref="custom-video_control-bg-outside">
              <span class="custom-video_control-bg-inside"
                    ref="custom-video_control-bg-inside"></span>
              <span class="custom-video_control-bg-inside-point"
                    ref="custom-video_control-bg-inside-point"></span>
            </div>
          </div>
          <!-- 声音 -->
          <div class="custom-video_control-voice">
            <!-- <van-icon class="custom-video_control-voice-play" name="volume-o" /> -->
            <div class="custom-video_control-voice-bg" ref="custom-video_control-voice-bg"
                 @mousedown="handleVolPrograssDown" @mousemove="handleVolPrograssMove"
                 @mouseup="handleVolPrograssUp">
              <div class="custom-video_control-voice-bg-outside"
                   ref="custom-video_control-voice-bg-outside">
                <span class="custom-video_control-voice-bg-inside"
                      ref="custom-video_control-voice-bg-inside"></span>
                <span class="custom-video_control-voice-bg-point"
                      ref="custom-video_control-voice-bg-point"></span>
              </div>
            </div>
          </div>
          <!-- 时间 -->
          ...
          <!-- 全屏缩放 -->
          <van-icon class="custom-video_control-full" @click="handleScreen" name="enlarge" />
        </div>
      </transition>
    </div>
  </div>
...
export default {
  ...
  methods: {
    initMedaData() { // 初始化video相关事件
      ...
      //  监听页面全屏事件（解决Bug：安卓物理返回，页面退出全屏，video还是全屏）
      document.addEventListener('fullscreenchange', this.closeFull, false)
      document.addEventListener('mozfullscreenchange', this.closeFull, false)
      document.addEventListener('webkitfullscreenchange', this.closeFull, false)
      document.addEventListener('msfullscreenchange', this.closeFull, false)
    },
    // 判断是否是安卓物理返回事件 关闭全屏
    closeFull() {
      const flag = this.checkFull()
      if (flag) {
        this.times++
      }
      this.videoState.screenState = flag
      if (this.times > 1) {
        this.times = 0
        this.handleScreen()
      }
    ...
    getOffset(node, offset) { // 获取当前屏幕下进度条的左偏移量和右偏移量
      if (!offset) {
        offset = {}
        offset.left = 0
        offset.top = 0
    ...
    // 全屏
    fullScreen() {
      const ele = document.getElementById(this.infoData.id || 'custom-video_video')
      // W3C
      ...
      // video模拟全屏
      this.$refs['custom-video_container'].style.width = '100vw'
      this.$refs['custom-video_container'].style.height = '100vh'
    },
    // 退出全屏
    exitFullscreen() {
      const de = document.getElementById(this.infoData.id || 'custom-video_video')
      ...
      this.$refs['custom-video_container'].style.width = this.infoData.width
      this.$refs['custom-video_container'].style.height = this.infoData.height
    }
  }
}
</script>
